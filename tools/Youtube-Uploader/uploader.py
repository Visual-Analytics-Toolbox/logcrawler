...
CLIENT_SECRETS_FILE = os.path.abspath(
    config.get("authentication", "client_secrets_file")
)
OAUTH2_STORAGE_FILE = os.path.abspath(
    config.get("authentication", "oauth2_storage_file")
)
...
def refresh_token_with_retry(creds):
    ...
    while retry_count < MAX_RETRIES:
        try:
            ...
            logger.error(
                f"HttpError refreshing token (attempt {retry_count + 1}/{MAX_RETRIES}): status={e.resp.status}, content={e.content}"
            )
        except RefreshError as e:
            logger.error(
                f"RefreshError refreshing token (attempt {retry_count + 1}/{MAX_RETRIES}): {e}"
            )
        except urllib.error.URLError as e:
            logger.error(
                f"Network error refreshing token (attempt {retry_count + 1}/{MAX_RETRIES}): {e}"
            )
        except Exception as e:
            logger.error(
                f"Unexpected error refreshing token (attempt {retry_count + 1}/{MAX_RETRIES}): {e}"
            )
        retry_count += 1
        sleep_seconds = (
            2**retry_count
...
def get_authenticated_service():
    ...
    if os.path.exists(OAUTH2_STORAGE_FILE):
        try:
            ...
            logger.info(
                f"Loaded credentials data: token={creds_data.get('token', '')[0:10]}..., expiry={creds_data.get('expiry')}"
            )
            creds = Credentials.from_authorized_user_info(creds_data, SCOPES)
            logger.info(
                f"Existing credentials: token={creds.token[:10]}..., expiry={creds.expiry}, refresh_token={creds.refresh_token[:10] if creds.refresh_token else 'None'}..."
            )
            ...
            if not creds.refresh_token:
                logger.warning(
                    "No refresh token available, forcing new authentication."
                )
                should_refresh = True
            elif creds.expiry:
                expiry_aware = (
                ...
                logger.info(
                    f"Token expiry: {creds.expiry}, time to expiry: {time_to_expiry}"
                )
                should_refresh = (
                    creds.expired
                    or time_to_expiry.total_seconds()
                    < 600  # Refresh if less than 10 minutes remaining
                    or time_to_expiry.total_seconds()
                    <= FORCE_TOKEN_REFRESH_DAYS
                    * 24
                    * 60
                    * 60  # Refresh if within refresh window
            ...
            if should_refresh and creds and creds.refresh_token:
                ...
                if not success:
                    logger.error(
                        "Token refresh failed after retries, forcing new authentication."
                    )
                    os.remove(OAUTH2_STORAGE_FILE)
                    creds = None
                elif not creds.valid:
                    logger.warning(
                        "Refreshed token is still invalid, forcing new authentication."
                    )
                    os.remove(OAUTH2_STORAGE_FILE)
                    creds = None
            ...
            logger.error(
                f"Invalid or corrupted credentials file ({e}), initiating new authentication."
            )
            os.remove(OAUTH2_STORAGE_FILE)
            creds = None
        except Exception as e:
            logger.error(
                f"Unexpected error loading credentials ({e}), initiating new authentication."
            )
            os.remove(OAUTH2_STORAGE_FILE)
            creds = None
    ...
    if not creds or not creds.valid:
        logger.info(
            "No valid credentials found, initiating manual authentication for headless system."
        )
        ...
        logger.info(
            f"Please visit this URL on a device with a browser to authorize the application: {authorization_url}"
        )
        print(
        ...
        try:
            ...
            logger.info(
                f"Credentials obtained: token={creds.token[:10]}..., expiry={creds.expiry}, refresh_token={creds.refresh_token[:10] if creds.refresh_token else 'None'}..."
            )
            if not creds.expiry:
                logger.warning(
                    "No expiry set after initial authentication, setting manually."
                )
                creds.expiry = datetime.now(timezone.utc) + timedelta(
                    seconds=3600
                )  # Set to 1 hour
            with open(OAUTH2_STORAGE_FILE, "w") as token:
                json.dump(json.loads(creds.to_json()), token)
                logger.info(
                    f"Credentials saved to {OAUTH2_STORAGE_FILE}, expiry={creds.expiry}"
                )
        except Exception as e:
    ...
    if creds and (not creds.valid or creds.expired) and creds.refresh_token:
        logger.info(
            "Credentials invalid or expired but refresh token available, attempting final refresh."
        )
        success = refresh_token_with_retry(creds)
        if not success:
            logger.error(
                "Final refresh attempt failed. Please re-authenticate manually."
            )
            os.remove(OAUTH2_STORAGE_FILE)
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--longitude", help="Longitude of the video location", type=float
    )
    parser.add_argument("--language", help="Language of the video", default="en")
    parser.add_argument(
        "--playlistId", help="ID of the playlist where the video should be added"
    )
    parser.add_argument("--thumbnail", help="Path to the thumbnail image file")
    parser.add_argument(
        "--license",
        choices=["youtube", "creativeCommon"],
        help="License of the video",
        default="youtube",
    )
    parser.add_argument(
        "--publishAt", help="ISO 8601 timestamp for scheduling video publish time"
    )
    parser.add_argument(
        "--publicStatsViewable",
        action="store_true",
        help="Whether video statistics should be public",
        default=False,
    )
    parser.add_argument(
        "--madeForKids",
        action="store_true",
        help="Set if the video is made for kids",
        default=False,
    )
    parser.add_argument("--ageGroup", help="Age group for the video (e.g., 'age18_24')")
    parser.add_argument(
        "--gender", help="Gender targeting for the video ('male', 'female')"
    )
    parser.add_argument(
        "--geo",
        help="Geographic targeting (comma-separated ISO 3166-1 alpha-2 country codes)",
    )
    parser.add_argument(
        "--defaultAudioLanguage", help="Default audio language for the video"
    )

    auth_group = parser.add_argument_group(
        "Authentication or debugging related options"
    )
    auth_group.add_argument(
        "--no-upload",
        action="store_true",
        help="Only authenticate, do not upload the video",
    )
    auth_group.add_argument(
        "--force-refresh", action="store_true", help="Force token refresh for debugging"
    )
    ...
    if not args.no_upload and not args.videofile:
        sys.exit(
            "Please specify a valid file using the --videofile= parameter if not using --no-upload."
        )
