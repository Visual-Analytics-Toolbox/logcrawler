    ...
    for game in games:
        ...
        for video in all_videos:
            ...
            if Path(video_path).suffix != ".mp4":
                logging.error(f"Video file name does not match expected format {video_path}")
                continue
            ...
            try:
                ...
                logging.error(f"error occured when trying to insert video  {video_path}:{e}")
                continue
            ...
            try:
                ...
                logging.error(f"error occured when trying to insert game {video_path}:{e}")