...
def input_experiment_gamelogs(log_root_path, client):
    logging.info("################# Input Log Data From Experiments #################")
    experiments = client.experiment.list(type="Gamelog")
    for experiment in sorted(experiments, key=sort_key_fn):
        logging.info(f"{experiment.id}: {experiment.name}")
        log_folder_path = Path(log_root_path) / experiment.experiment_folder
