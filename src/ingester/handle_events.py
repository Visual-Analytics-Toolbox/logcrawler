...
def input_events(log_root_path, client):
    ...
    for event in sorted(all_events):
        if event.name in event_list:
            logger.info(f"processing {event.name} event")
            ...
            try:
                ...
                logger.debug(f"inserted event: {new_event}")
    ...
    if len(event_list) > 0:
        for event in event_list:
            logger.warning(f"Couldn't find the {event} event folder")
...
def input_lab_events(log_root_path, client):
    ...
    for event in sorted(all_events):
        if event.name in lab_experiments:
            logger.debug(f"handle {event.name} event")
