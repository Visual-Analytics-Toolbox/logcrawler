...
day = ["2018-06-18", "2018-06-19", "2018-06-20", "2018-06-21"]
...
def get_base_folder_name(game):
    return (
        game[0] + "_" + game[1] + "_" + game[2] + "_vs_" + game[3] + "_" + "half1",
...
def candiate_matches(candidate, game):
    if game[0] in candidate:
        try:
            if all(x in candidate.casefold() for x in search_keys[game[2]]) and all(
                x in candidate.casefold() for x in search_keys[game[3]]
            ):
                return True
...
def found_candidate(c, dest, game):
    ...
    if candiate_matches(c_file, game):
        if "1st" in c_file:
            ...
            print(
                "Error: " + c_file + " in " + c_folder + " has no half", file=sys.stderr
            )
            return False
...
def played_vs(candidate, date, opponent):
    ...
    try:
        if (
            all(x in candidate.casefold() for x in search_keys[opponent])
            and "Test".casefold() not in candidate.casefold()
        ):
            return True
    except KeyError as e:
        print("Error: can't get search key for team " + str(e), file=sys.stderr)
...
def rename_and_move(src, dst):
    ...
    try:
        ...
        print(
            "Error: couldn't delete folder " + str(e) + ". Directory not empty.",
            file=sys.stderr,
        )
...
def find_game_logs(candidates, dest, game):
    ...
    if not cs:
        print(
            "Error: couldn't find a folder for a game vs "
            + opponent
            + " on "
            + game[0],
            file=sys.stderr,
    ...
    if len(cs) > 1:
        print(
            "Error: couldn't find unique folder for a game vs "
            + opponent
            + " on "
            + game[0],
            file=sys.stderr,
    ...
    if os.path.exists(cs[0] + "/half1_timeout"):
        rename_and_move(
            cs[0] + "/half1_timeout", dest + "/" + game_folder[0] + "_to1/game_logs"
        )
    if os.path.exists(cs[0] + "/half2_timeout"):
        rename_and_move(
            cs[0] + "/half2_timeout", dest + "/" + game_folder[1] + "_to2/game_logs"
        )
...
def main():
    ...
    game_log_src_folders = [
        d
        for d in os.listdir(src + "/game_logs")
        if os.path.isdir(os.path.join(src + "/game_logs", d))
    ]
    game_log_src_folders = list(
        map(lambda x: os.path.join(src, "game_logs", x), game_log_src_folders)
    )
    ...
    for game in games:
        ...
        if game[2] in ("Berlin United", "DoBerMan") or game[3] in (
            "Berlin United",
            "DoBerMan",
        ):
